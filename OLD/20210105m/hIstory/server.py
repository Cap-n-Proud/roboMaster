...
def sendProgramToArduino():
    ...
    sendArduinoCommand("setBrightness " + str(data["lightBrightness"]))
    sendArduinoCommand("setLightRGB " + str(data["RGB"]) + "\n")
    print("Program sent")
...
if __name__ == "__main__":
    ...
    checkT = RepeatedTimer(
        2, checkLights, currentProgram, dataJSON
    )  # it auto-starts, no need of rt.start()
    checkP = RepeatedTimer(
        currentProgram["pumpStartEvery"], pump, currentProgram["pumpRunTime"], dataJSON
    )  # it auto-starts, no need of rt.start()
